...
## This function generates trajectories for unitary AND locations FIXED
def get_trajectories_fixed(L,depth,Q,p,seed,shots,scrambled=False):
    if not scrambled:
        filedir = 'data/measurement_data_fixed/seed='+str(seed)
    else:
        filedir = 'data/measurement_data_fixed_scrambled/seed='+str(seed)
    if not os.path.isdir(filedir):
        os.makedirs(filedir)
    ...
    if len(measurement_array) == shots:
        return 
    
    required_shots = shots - len(measurement_array)

    new_trajectories = quantum_trajectories(L,depth,Q,p,required_shots,m_locs,param_list,scrambled=scrambled)
...
def collect_fixed_data(L_list,p_list,seed,samples,depth_ratio=1,scrambled=False):
    for L in L_list:
        for p in p_list:
            start = time.time()
            get_trajectories_fixed(L,L*depth_ratio,L//2,p,seed=seed,shots=samples,scrambled=scrambled)
            get_trajectories_fixed(L,L*depth_ratio,L//2 - 1,p,seed=seed,shots=samples,scrambled=scrambled)
            print(L,p,time.time()-start)

p_list = [0.05,0.1,0.13,0.16,0.2,0.25,0.3,0.4]
L_list = [6,8,10,12,14]
collect_fixed_data(L_list,p_list,2,2000,depth_ratio=2,scrambled=True)