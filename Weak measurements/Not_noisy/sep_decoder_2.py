...
def get_sep_data(Q,L,p,depth_ratio,scrambled):
    ...
    p_suc = []
    seed=1
    file_dir = 'Weak measurements/data/measurement_data_all_qubits'
    neel_state = True
    if scrambled:
        neel_state = False
    ...
    for data in data_raw:
        p_suc.extend([sep_dynamics_2(data[0],Q,p,neel_initial_state=neel_state)]*data[1])
    return p_suc
...
for L in L_list:
    ...
    for p in p_list:
        ...
        for Q in [L//2,L//2-1][:]:
            start = time.time()
            p_suc_dic[L][p][Q] = get_sep_data(Q,L,p,depth_ratio=depth_ratio,scrambled=scrambled)
