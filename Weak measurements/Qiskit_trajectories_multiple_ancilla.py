...
import pickle
import os
import circuit_generation_multiple_ancilla
...
def quantum_trajectories(L,depth,Q,theta,shots,m_locs,p_depo_1,p_depo_2,basis_gate_1_site,basis_gate_2_site,scrambling_type,is_noisy,circ=None,scrambling_param = [],param_list=[],seed=None):
    ...
    noise = circuit_generation_multiple_ancilla.noise_model(p_depo_1=p_depo_1,p_depo_2=p_depo_2,basis_gates_1_site=basis_gate_1_site.copy(),basis_gates_2_site=basis_gate_2_site.copy())
    ...
    if circ is None:
        _,_,circ = circuit_generation_multiple_ancilla.generate_u1mrc(L,depth,m_locs,params=param_list,Q=Q,theta=theta,debug=False,scrambling_type=scrambling_type,scrambling_param=scrambling_param)
...
#----------------------------------------------------------------------------------------------------#
    def set_circuit_variables():
        ...
        root_dir = 'Weak measurements/data/multiple_ancilla/circ_data/'
        if t_scram == 2:
            root_dir = 'Weak measurements/t_scram=2/data/multiple_ancilla/circ_data/'
...
#----------------------------------------------------------------------------------------------------#
    ...
    scr_param, param_list = circuit_generation_multiple_ancilla.get_circuit_parameters(seed=seed,t_scram=t_scram,L=L,depth=depth,scrambling_type=scrambling_type)
...
def collect_fixed_data(L_list,p_list,seed,samples,p_depo_1,p_depo_2,t_scram,scrambling_type,is_noisy,depth_ratio=1):
    ...
    def get_filedir():
        ...
        filedir = 'Weak measurements/data/multiple_ancilla/qiskit_data/measurement_data_all_qubits'+ scrambling_label + noisy_label + depth_label+'/'
        if t_scram == 2:
            filedir = 'Weak measurements/t_scram=2/data/multiple_ancilla/qiskit_data/measurement_data_all_qubits'+ scrambling_label + noisy_label + depth_label+'/'
...
p_list = list(np.around(np.array([0.001,0.01,0.02,0.05,0.1,0.15])*np.pi/2,3)) + p_list
...
depth_ratio = 0.5
t_scram = 5
collect_fixed_data(L_list,p_list,seed=1,samples=0,depth_ratio=depth_ratio,p_depo_1=p_depo_1,p_depo_2=p_depo_2,scrambling_type=scrambling_type,is_noisy=is_noisy,t_scram=t_scram)
