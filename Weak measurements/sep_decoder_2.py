...
def get_sep_data(Q,L,p,depth_ratio,scrambling_type,is_noisy):
...
for L in L_list:
    ...
    for p in p_list:
        ...
        for Q in [L//2,L//2-1][:]:
            start = time.time()
            p_suc_dic[L][p][Q],temp = get_sep_data(Q,L,p,depth_ratio=depth_ratio,scrambling_type=scrambling_type,is_noisy=is_noisy)
