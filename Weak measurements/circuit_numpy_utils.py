"""
    This file contains utility functions to convert the qiskit circuit to numpy arrays.

    This will be mainly used for quantum and biased decoder
"""
...
def get_diffusion_constants(circuit_layer):
    Diff = []
    for t in range(len(circuit_layer)):
        Diff.append([])
        for gate in circuit_layer[t]:
            x = gate[0]
            y = gate[1]
